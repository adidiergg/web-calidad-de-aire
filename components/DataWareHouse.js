...
import { DataGrid, GridRowsProp, GridColDef } from '@mui/x-data-grid';
  ...
  */
  ...
  const columns = [
    { field: '0', headerName: 'Fecha' , flex:1 },
    { field: '1', headerName: 'Indice de calidad de aire maximo' ,flex:1 },
  ];
...
export default function DataWareHouse() {
    ...
    async function getDataCentro() {
      try {
        const response = await fetch('http://127.0.0.1:8000/datawarehouse/centro', {
          method: 'GET',
          headers: {
            accept: 'application/json',
  ...
  async function getDataNoreste() {
    try {
      const response = await fetch('http://127.0.0.1:8000/datawarehouse/noreste', {
        method: 'GET',
        headers: {
          accept: 'application/json',
...
async function getDataNoroeste() {
  try {
    const response = await fetch('http://127.0.0.1:8000/datawarehouse/noroeste', {
      method: 'GET',
      headers: {
        accept: 'application/json',
...
async function getDataSureste() {
  try {
    const response = await fetch('http://127.0.0.1:8000/datawarehouse/sureste', {
      method: 'GET',
      headers: {
        accept: 'application/json',
...
async function getDataSuroeste() {
  try {
    const response = await fetch('http://127.0.0.1:8000/datawarehouse/suroeste', {
      method: 'GET',
      headers: {
        accept: 'application/json',
  ...
  try {
    ...
    return isClean===true ? (
        ...
        <div style={{ height: 500, width: '100%' }}>
            <DataGrid rows={dataWareHouseCentro} columns={columns} />
        </div>
        ...
        <div style={{ height: 500, width: '100%' }}>
            <DataGrid rows={dataWareHouseNoreste} columns={columns} />
        </div>
        ...
        <div style={{ height: 500, width: '100%' }}>
            <DataGrid rows={dataWareHouseNoroeste} columns={columns} />
        </div>
        ...
        <div style={{ height: 500, width: '100%' }}>
            <DataGrid rows={dataWareHouseSureste} columns={columns} />
        </div>

        <br/>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
             <h3 className="text-2xl font-semibold text-gray-900">Suroeste</h3>
        </div>
        <div style={{ height: 500, width: '100%' }}>
            <DataGrid rows={dataWareHouseSuroeste} columns={columns} />
        </div>
      ...
      <a href="limpieza" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Datawarehouse vacio, ir a proceso de limpieza</a>
